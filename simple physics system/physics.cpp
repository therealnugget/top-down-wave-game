...
Node<RigidBody*>* Physics::SubscribeEntity(const std::initializer_list<const char*>basePaths, std::initializer_list<FVector2> _narrowPhaseVertices, FVector2 startPos, IntVec2 size, std::initializer_list<FVector2> _centreOfRot, FVector2 _centreOfRotNPVert, IntVec2 _renderOffset, FVector2 initVel, float angle, float mass) {
	RigidBody *rb = new RigidBody(startPos, initVel, angle, basePaths, size, mass, _narrowPhaseVertices, _centreOfRot, _centreOfRotNPVert, _renderOffset);
