...
void SwordGuy::CollisionCallback(Collision* collision) {
    if (!enabled) return;
    if (collision->CompareTag(Main::Tag::player) && !plrColOnFrm) {
...
void SwordGuy::ResetIfAttkFin(int animation) {
    if (animFinished) {
        if (curAnim == dash) {
            Player::TakeDamage(damage);
            rb->newPosition.x += dashPositionAdd * Math::Sign(GetToPlr().x);
        }
