...
void SwordGuy::CollisionCallback(Collision& collision) {
    if (entity->GetCurAnim() == hurt && !entity->AnimFinished() || !collision.CompareTag(Main::Tag::playerAttack)) return;
    cout << health << endl;
...
void SwordGuy::Update(void) {
    ...
    if (plrDistSqr < dashDistSqr) {
        if (attackDecision == -1) attackDecision = Main::GetRandInt(overhead, swing, dash);
        ...
        else if (plrDistSqr < attackDistSqr && attackDecision != -1) {
